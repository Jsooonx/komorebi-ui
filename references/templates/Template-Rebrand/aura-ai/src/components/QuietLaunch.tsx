...
export default function QuietLaunch() {
  ...
  return (
    <section className="quiet-launch" id="contact" aria-labelledby="quiet-launch-title">
      <motion.div
        className={`quiet-launch__scene${active ? " is-active" : ""}`}
      ...
      >
        <img
          src="/assets/aura/aura-quiet-launch.png"
          alt="A calm glass threshold carrying Aura's signal through a sunlit forest clearing"
        />
        <div className="quiet-launch__wash" aria-hidden="true" />
        <motion.div
          className="quiet-launch__threshold-glow"
          aria-hidden="true"
          animate={{ opacity: active ? 1 : 0.28, scale: active ? 1.08 : 1 }}
        ...
        <div className="quiet-launch__intro">
          ...
          >
            Start with one conversation. Let it earn the next.
          </motion.h2>
        </div>
        <div className="quiet-launch__marks" aria-label="Launch readiness checks">
          {proofMarks.map((mark) => (
            <motion.button
              ...
              type="button"
              key={mark.id}
              className={`quiet-launch__mark ${mark.position}${activeMark === mark.id ? " is-active" : ""}`}
            ...
            >
              {activeMark === mark.id && (
                <motion.span
                  className="quiet-launch__mark-highlight"
                  layoutId="aura-launch-active-highlight"
        ...
        <motion.p
          className="quiet-launch__active"
          key={active?.id ?? "resting"}
          initial={{ opacity: 0, y: 6 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.22, ease }}
        >
          {active?.detail ?? "A considered rollout, not a leap of faith."}
        </motion.p>
      </motion.div>
      ...
      <motion.div
        className="quiet-launch__cta"
        initial="hidden"
        whileInView="visible"
        viewport={viewport}
        variants={reveal}
        transition={{ duration: 0.76, ease, delay: 0.14 }}
      >
        <p>Bring your knowledge, your standards, and one customer moment worth improving.</p>
        <button type="button">
          Plan your first conversation <span>↗</span>
        </button>
      </motion.div>
    </section>
  );
