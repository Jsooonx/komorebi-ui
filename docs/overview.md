...
`BLOCKS_MANIFEST` contains the 23 layout blocks shown in `/blocks`, including Siena Parallax, Oliver Parallax, Atlas Horizontal Parallax, and Depth Lens Parallax in the Parallax category, four Preloader blocks, three Product Showcase blocks, and Voice Index Testimonials in the Testimonials & Social Proof category. Blocks expose category routes such as `/blocks/parallax`, `/blocks/preloader`, `/blocks/product-showcase`, and `/blocks/testimonials`, and each block has a fullscreen page route such as `/blocks/parallax/siena-parallax`, `/blocks/preloader/morphing-lens-preloader`, `/blocks/product-showcase/orbit-workspace-showcase`, or `/blocks/testimonials/voice-index-testimonials`.
