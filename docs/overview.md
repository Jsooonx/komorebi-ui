...
`BLOCKS_MANIFEST` contains the 22 layout blocks shown in `/blocks`, including Siena Parallax, Oliver Parallax, Atlas Horizontal Parallax, and Depth Lens Parallax in the Parallax category, four Preloader blocks, and three Product Showcase blocks. Blocks expose category routes such as `/blocks/parallax`, `/blocks/preloader`, and `/blocks/product-showcase`, and each block has a fullscreen page route such as `/blocks/parallax/siena-parallax`, `/blocks/preloader/morphing-lens-preloader`, `/blocks/product-showcase/focus-tour-showcase`, or `/blocks/product-showcase/orbit-workspace-showcase`.
