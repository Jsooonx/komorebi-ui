...
*   **Aura Connected Context:** Aura's fourth section explains where an informed answer comes from. A generated forest-gold visual shows five distinct glass source nodes converging into an aqua Aura context core; each source label has a local hover/focus state and explanatory microcopy, avoiding a generic integrations logo wall.
