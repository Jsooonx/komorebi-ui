...
- The required completion order is: make changes -> update `docs/` -> run memory sync -> push GitHub.
- Private memory artifacts inside `obsidian_memory_vault/` and the sync implementation in `scripts/sync_memory.py` must remain unpushed.
