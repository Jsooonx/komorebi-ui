...
- `Components` refers to the catalog route at `/components`
- `Bento-Showcase` refers only to the landing-page highlight grid
- not every catalog component needs to appear inside the Bento-Showcase
...
## Catalog-Only Components

Some components are available in the `Components` catalog without being featured in the landing-page Bento-Showcase.

- `Pixel Shimmer Card` is currently catalog-only and is registered through `src/lib/components-manifest.ts`.
