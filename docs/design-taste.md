...
- A fixed headline slot may still use a local text morph: overlap the outgoing and incoming strings inside an explicitly reserved text height, then use a small directional offset, opacity, and clip reveal. The headline can feel continuous while its card, metadata, and baseline remain completely still.
