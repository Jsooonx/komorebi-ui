...
- A split CTA may use one focused visual proof frame, but the visual must clarify the same promise as the copy rather than compete with it. Prefer original CSS composition or a carefully selected clean image; avoid borrowed in-image text, dashboard screenshots, or a second action inside the media panel.
