...
- Catalog controls and other repeated UI interactions must respond in roughly 150–200ms with property-specific transitions, a subtle press scale, and a strong ease-out. Avoid `transition-all`, long list entrance motion, and hover-only feedback that misfires on touch; decorative block animation stays scoped to the block itself and respects reduced-motion preferences.
- Scroll-linked blocks keep their structural anchors synchronous with scroll: never spring-lag a pinned scene, and never animate layout geometry such as `top`, `width`, or `height` on every scroll frame. Preserve the layout and use transform/opacity plus nested `clip-path` for visual morphing. Preloaders and preview-only sequences must provide a short opacity-first `prefers-reduced-motion` completion path.
- Siena is intentionally a slow editorial scroll: its fullscreen scene uses a long `300dvh` travel range, while the catalog preview preserves the same sequence in a compressed but still unhurried range.
- Parallax blocks should remain distinct patterns: Siena focuses on image-to-card transformation, Oliver on staggered gallery depth, Atlas on a horizontally traveling chapter track, and Depth Lens on layered cinematic depth. Oliver, Atlas, and Depth Lens use the locally curated `public/assets/shared/editorial/pinterest/` collection; Siena retains its dedicated generated monochrome image. Depth Lens uses a clipped atmospheric background, midground texture, focal image, and foreground frame with restrained spring-smoothed scroll offsets; it should stay DOM/Framer-based and avoid WebGL, drag controls, and forced snap points.
...
- Version Fold Showcase is the non-workspace Product Showcase pattern: a single product artifact may expose a small set of click-selected states without becoming a dashboard, carousel, or scroll scene. Keep its outer frame, size, and center position fixed. Use a shared local selection indicator and overlap fixed content slots for short directional opacity/position transitions; use a direct duration/ease transition rather than a spring when the state must stop immediately after the click.
- Dependency Weave Showcase is the relationship-led Product Showcase pattern. A small 2D graph may use original generated imagery as meaningful project artifacts, while DOM/SVG owns all relationships and interaction. Keep the graph stable, reveal only the hovered or focused node's direct upstream/downstream paths, and use a fixed central impact panel for local content swaps. On pointer leave, return to a neutral state with every active path turned off rather than preselecting an artifact. Do not turn it into an infinite canvas, draggable graph, orbit, dashboard, or autoplay animation; the selected path must stop immediately after hover.
- For pinned product scenes, separate fixed header, flexible canvas, and caption regions in the viewport grid. Scroll may drive local fragments, but it must not move the scene frame or place critical labels in an absolute footer that can be cut off at shorter viewport heights.
- When a scroll-driven phase changes copy inside a centered artifact, lock the artifact's outer height before animating its local content. Never let copy length trigger a re-centering layout reflow that reads as page jitter.
- For phase-swapped copy inside a fixed artifact, reserve a fixed inner content slot and layer the outgoing and incoming text absolutely inside it. Animate only local opacity and a small directional offset; do not use layout-aware replacement modes or blur filters when readable typography needs to remain perfectly stable.
- If a scroll-driven text transition still reads as jitter, prefer a direct text replacement in the fixed slot. Do not animate the surrounding artifact, metadata, index, state label, or baseline merely to make the copy change feel more decorative.
- A designated stable artifact must not receive scroll-linked `scale`, `clipPath`, width, height, or positional transforms. If the requirement is "only text changes," keep its entire frame as a plain static DOM layer and apply motion solely to surrounding secondary fragments.
- A fixed headline slot may still use a local text morph: overlap the outgoing and incoming strings inside an explicitly reserved text height, then use a small directional offset and opacity. Avoid `clip-path` on serif headline glyphs, because the reveal can visibly cut ascenders or descenders during the transition. The headline can feel continuous while its card, metadata, and baseline remain completely still.
- Scroll-linked scenes must use raw scroll progress when the visual is expected to stop immediately with the pointer or trackpad. Do not spring-smooth the scroll motion value unless intentional follow-through is explicitly desired; that lag reads as residual page movement during fast scrolling.
- When a scroll scene needs headline morphing without residual movement, bind the overlapping headline layers directly to raw scroll progress. Avoid state-triggered spring transitions at scroll thresholds; they keep animating after a fast scroll has stopped.
