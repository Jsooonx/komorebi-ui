...
Voice Index Testimonials is the first Testimonials & Social Proof block. It treats social proof as an editorial archive rather than a product interface: a large quote changes in place as a vertical identity index is traced by natural scroll. Header, quote, rail, and closing note use explicit vertical regions so the oversized editorial copy never overlaps the section title. Four original voices stay mounted, the active row receives only a local highlight, and the catalog/fullscreen surfaces use their own scroll containers without autoplay, drag, snap, nested Bento, or dashboard chrome. The quote may use a light editorial serif while names, roles, outcomes, and controls remain sans-serif/mono.
