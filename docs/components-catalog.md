...
Usage Horizon Pricing is the second Pricing & Conversion block. It complements Plan Lens rather than repeating it: instead of choosing a tier, people explore a clear team-capacity value metric through four compact horizons. The active marker morphs along one stable track while only its local forecast, included capacity, and transparent monthly shape change. It stays a pre-checkout pricing decision surface with no dashboard chrome, transaction form, decorative media, or autoplay; the catalog and fullscreen share the same direct DOM/CSS interaction.
