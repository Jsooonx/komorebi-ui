...
Version Fold Showcase is the fourth Product Showcase block. It avoids a workspace, canvas, and scroll narrative in favor of one centered release artifact with a direct title. Click-selecting Draft, Review, or Ready moves a shared selection indicator and locally replaces the document copy, marks, and state label inside fixed slots. Fullscreen gives this artifact a deliberately larger focal footprint, while the catalog keeps a compact readable version. In both surfaces, the artifact, its height, and its viewport position remain static, so the transition reads as one record resolving rather than a new page appearing.
Dependency Weave Showcase is the fifth Product Showcase block. It treats project dependencies as the product visual: four original generated editorial artifacts form a compact 2D graph around one stable impact panel. Fullscreen gives the graph a larger focal footprint while the catalog remains compact. Hovering or keyboard-focusing a project lights only its upstream and downstream SVG paths and locally changes its blockers, unlocks, milestone, and contextual copy; leaving the graph turns every path off and restores a neutral trace prompt. It deliberately avoids scroll scenes, orbit motion, drag canvases, and generic dashboard chrome.
...
Community Mosaic Testimonials is the third Testimonials & Social Proof block. It uses a naturally scrollable three-column wall of 12 fictional community voices, each with a generated portrait and a deliberately varied feature, standard, compact, or quiet card treatment. Staggered column offsets, density, padding, type scale, and surface contrast create the masonry rhythm instead of a uniform grid. Unlike Voice Index and Proof Ledger, it is not a selection surface: the cards stay readable as a collective proof wall, with no autoplay, switching state, entrance sequence, carousel, dashboard chrome, or nested Bento. Hover only brightens and lifts the individual card and portrait under the pointer; catalog keeps the mosaic scrollable within its viewport while fullscreen renders it as a complete page.
