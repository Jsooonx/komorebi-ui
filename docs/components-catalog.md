...
Dependency Weave Showcase is the fifth Product Showcase block. It treats project dependencies as the product visual: four original generated editorial artifacts form a compact 2D graph around one stable impact panel. Selecting a project lights only its upstream and downstream SVG paths and locally changes its blockers, unlocks, milestone, and contextual copy. It deliberately avoids scroll scenes, orbit motion, drag canvases, and generic dashboard chrome.
