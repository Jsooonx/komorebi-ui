...
Aperture Canvas Showcase is the fourth Product Showcase block. It replaces dashboard chrome with an off-white product canvas: a field note, source image, and outcome signal gather around one persistent decision brief as natural scrolling progresses. Its source tile uses an original generated tracing-paper image with a restrained cobalt accent; the catalog has its own compact click-driven arrangement surface, while fullscreen uses a 320dvh sticky canvas. The fullscreen canvas is divided into fixed header, canvas, and caption regions, with compact phase captions kept to one readable line at the intended desktop width. Its central decision brief has a fixed position, width, height, scale, and clip frame, plus a deliberately generous fixed headline slot so serif descenders never clip; scroll changes only its headline and index text while the surrounding fragments move directly with raw scroll progress. The interaction illustrates messy input becoming a shareable decision rather than another workspace, guided tour, or spatial orbit.
