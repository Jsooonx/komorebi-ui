...
- Component cards use compact catalog adapters from `src/components/components-preview-elements/`. They render the canonical element unchanged, then crop and reposition its interaction area so the catalog shell remains the only visible card container.
...
- `src/components/components-preview-elements/` contains compact catalog adapters. They may resize, crop, and reposition the canonical component, but must never change its implementation or recreate its interaction.
