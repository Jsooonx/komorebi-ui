...
Plan Lens Pricing is the first Pricing & Conversion block. It keeps three concise fictional tiers on one stable comparison surface: hovering, focusing, or tapping a tier moves a shared selection lens, expands the active plan locally, and reveals its outcome-led scope. It uses DOM/CSS capacity markers rather than decorative imagery; the catalog remains compact while fullscreen centers the complete, non-scrollable composition.
