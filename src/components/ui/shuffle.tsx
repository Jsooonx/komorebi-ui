...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        // Manual text splitting replacement for gsap SplitText plugin to avoid premium subscription dependency
        ...
        words.forEach(word => {
          if (/^\s+$/.test(word)) {
            ...
            spaceNode.innerHTML = word.replace(/ /g, '&nbsp;');
            spaceNode.className = 'shuffle-space inline-block';
            el.appendChild(spaceNode);
          } else {
            const wordSpan = document.createElement('span');
            wordSpan.className = 'shuffle-word inline-block';
            word.split('').forEach(char => {
              const charSpan = document.createElement('span');
              charSpan.className = 'shuffle-char inline-block';
        ...
        chars.forEach(ch => {
          ...
          // Add vertical safety padding to prevent tall ascenders/descenders from clipping,
          // and cancel it out using negative vertical margins. Align to baseline.
          wrap.className = 'inline-block overflow-hidden text-left py-4 -my-4';
  ...
  const baseTw = 'inline-block whitespace-normal break-words will-change-transform select-none';
